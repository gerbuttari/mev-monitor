...
async function mevLogin(usuario, clave) {
  ...
  if (body.toLowerCase().includes('usuario incorrecto')) throw new Error('Usuario incorrecto.');
  return allCookies;
}
...
function inRange(s, desde, hasta) {
  const d = pd(s); if (!d) return false;
  const a = pd(desde), b = pd(hasta);
...
async function getActuaciones(cookie, nid) {
  ...
  $('table tr').each((_, row) => {
    const cells = $(row).find('td');
    const f = cells.eq(0).text().trim(), d = cells.eq(1).text().trim();
    if (isDateStr(f) && d) acts.push({ fecha: f, descripcion: d });
  });
  return acts;
}
...
async function scanMEV({ usuario, clave, fechaDesde, fechaHasta, emailDestino, smtpConfig }) {
  ...
  const todas = [];
  for (const o of orgs) { try { todas.push(...parseCausas(await getCausas(cookie, o.id), o.id)); await sleep(400); } catch(e) { console.error('[MEV]', o.id, e.message); } }
  console.log('[MEV] Causas: ' + todas.length);
  const conNov = todas.filter(c => inRange(c.ultimaNovedad, fechaDesde, fechaHasta));
  console.log('[MEV] Con novedad: ' + conNov.length);
  const det = [];
  for (const c of conNov) {
    try {
      const acts = (await getActuaciones(cookie, c.nidCausa)).filter(a => inRange(a.fecha, fechaDesde, fechaHasta));
      if (acts.length) det.push({ ...c, actuaciones: acts });
      await sleep(300);
    } catch(e) { console.error('[MEV] causa err', c.nidCausa, e.message); }
  }
